    ...
    public void getPageLinks(String URL, int depth, invertedIndex.Index5 index) {
       System.out.println("|| URL: [" + URL + "] --------  depth: " + depth + " fid=" + fid + " plinks=" + plinks + "\t|||| ");
        ...
        if ((!(links.contains(URL)))
            ...
            try {
                links.add(URL); // 1- add this URL to the visited list

                Document document = Jsoup.connect(URL).get(); // initialize the document element using the Jsoup library

                Elements linksOnPage = document.select("a[href]"); // 2- get all links of the page

                String docText = getText(document); // 3- get all paragraphs <p></p> elements from the page and accumulate them into a string

                SourceRecord sr = new SourceRecord(fid, URL, document.title(), docText); // build the SourceRecord
                sr.length = docText.length();
                sources.put(fid, sr); // 4- pass the docText for the inverted index with the doc id
                index.buildIndex(docText,fid);
                plinks++; // accumulator for the link in a sub-branch
                fid++; // current document id

                for (Element page : linksOnPage) {
                    getPageLinks(page.attr("abs:href"), depth + 1, index); // 5- handle all the page hyperlinks recursively with depth +1
                }
                plinks--;
            } catch (IOException e) {
